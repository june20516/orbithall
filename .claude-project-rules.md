...
### 개발 환경
- **Docker**: API 서버 및 PostgreSQL 실행
- **로컬 Go**: 테스트 실행용 (권장)
- 테스트는 로컬 Go에서 실행하는 것이 더 빠르고 편리함

### 배포
- **플랫폼**: Render (API) + Neon (Database)
...
- **대상**: slug 기반 웹 콘텐츠에 댓글 기능 제공 (프레임워크/빌드 방식 무관)
- **API 포트**: 8080
...
Orbithall은 slug 기반의 모든 웹 콘텐츠에 댓글 기능을 제공합니다.

### API 호출 예시
```typescript
// 웹사이트에서 API 호출 예시
fetch('http://localhost:8080/api/posts/your-post-slug/comments', {
  headers: {
...
- Docker Desktop 설치됨
- Go 1.25 로컬 설치됨 (테스트 실행용)
- PostgreSQL 로컬 설치 안 됨 (Docker 사용)
...
**최종 업데이트**: 2026-09-18
**프로젝트 단계**: 댓글 CRUD API 완료, 프로덕션 배포 완료 (Render + Neon)
