...
`pending/` 문서 이름은 `p{순서}-{작업번호}-{작업명}.md` 형식입니다. `p{순서}`는 선후관계와 우선순위를 반영한 진행 순서이고, `{작업번호}`는 작업을 식별하는 고유 번호입니다. 순서가 바뀌면 접두어만 바꾸고, 작업을 시작하면 접두어를 떼고 `active/`로 옮깁니다. 앞 번호가 빠져도(예: `p1` 시작 후 `p2`부터 남음) 남은 문서의 번호는 다시 매기지 않습니다.
