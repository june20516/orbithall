...
func CreateComment(db *sql.DB, postID int64, parentID *int64, authorName, password, content, ipAddress, userAgent string) (*models.Comment, error) {
	...
	row := db.QueryRow(query, postID, parentID, authorName, string(hashedPassword), content, ipAddress, userAgent)
...
func UpdateComment(db *sql.DB, commentID int64, content, ipAddress, userAgent string) error {
	query := `
		UPDATE comments
		SET content = $1,
			ip_address = $2,
	...
	result, err := db.Exec(query, content, ipAddress, userAgent, commentID)
	if err != nil {
...
func ListComments(db *sql.DB, postID int64, limit, offset int) ([]*models.Comment, int, error) {
	// 1단계: 최상위 댓글 총 개수 조회
	var total int
	err := db.QueryRow(`
		SELECT COUNT(*)
	...
	for _, comment := range comments {
		replies, err := getReplies(db, comment.ID)
		if err != nil {
...
// created_at ASC 순으로 정렬됩니다
func getReplies(db *sql.DB, parentID int64) ([]*models.Comment, error) {
	query := `
		SELECT id, post_id, parent_id, author_name, author_password, content, ip_address, user_agent, is_deleted, created_at, updated_at, deleted_at
		FROM comments
		WHERE parent_id = $1
