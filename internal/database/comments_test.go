...
// cleanupComments는 테스트 후 comments 테이블을 정리합니다
...
func TestCreateComment(t *testing.T) {
	...
	t.Run("최상위 댓글 생성 성공", func(t *testing.T) {
		...
		err = db.QueryRow(`
			...
			VALUES ($1, 'test-post', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
		...
		comment, err := CreateComment(db, postID, nil, authorName, password, content, ipAddress, userAgent)
	...
	t.Run("대댓글 생성 성공 (1depth)", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-reply', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
		...
		// When: 대댓글 생성
		comment, err := CreateComment(db, postID, &parentCommentID, "Reply Author", "replypass", "Reply content", "10.0.0.2", "Agent")
	...
	t.Run("대댓글의 대댓글 생성 실패 (2depth 금지)", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-depth', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
		...
		// When: 대댓글의 대댓글 생성 시도
		_, err = CreateComment(db, postID, &replyCommentID, "Nested Reply", "pass", "Nested", "10.0.0.3", "Agent")
	...
	t.Run("존재하지 않는 부모 댓글 ID로 생성 실패", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-invalid-parent', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
		...
		// When: 존재하지 않는 부모 ID로 대댓글 생성 시도
		_, err = CreateComment(db, postID, &nonExistentParentID, "Author", "pass", "Content", "10.0.0.1", "Agent")
...
func TestGetCommentByID(t *testing.T) {
	...
	t.Run("존재하는 댓글 조회 성공", func(t *testing.T) {
		...
		// 댓글 생성
		comment, err := CreateComment(db, postID, nil, "Test Author", "password123", "Test content", "192.168.1.1", "Mozilla/5.0")
		if err != nil {
			t.Fatalf("failed to create comment: %v", err)
		}

		// When: GetCommentByID 호출
		retrieved, err := GetCommentByID(db, comment.ID)
	...
	t.Run("존재하지 않는 댓글 조회 시 nil 반환", func(t *testing.T) {
		...
		// When: GetCommentByID 호출
		comment, err := GetCommentByID(db, nonExistentID)
	...
	t.Run("삭제된 댓글도 조회됨 (is_deleted=true)", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-deleted', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
...
func TestUpdateComment(t *testing.T) {
	...
	t.Run("댓글 수정 성공", func(t *testing.T) {
		...
		comment, err := CreateComment(db, postID, nil, "Test Author", "password123", originalContent, originalIP, originalUA)
		if err != nil {
			t.Fatalf("failed to create comment: %v", err)
		}
		...
		newIP := "10.0.0.1"
		newUA := "Chrome/120.0"
		err = UpdateComment(db, comment.ID, newContent, newIP, newUA)
		...
		// 수정된 댓글 조회하여 검증
		updated, err := GetCommentByID(db, comment.ID)
		if err != nil {
	...
	t.Run("존재하지 않는 댓글 수정 시 에러 반환", func(t *testing.T) {
		...
		// When: 수정 시도
		err := UpdateComment(db, nonExistentID, "New content", "10.0.0.1", "Agent")
	...
	t.Run("삭제된 댓글 수정 시 에러 반환", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-deleted-update', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
		...
		// When: 삭제된 댓글 수정 시도
		err = UpdateComment(db, commentID, "New content", "10.0.0.2", "Agent2")
...
func TestDeleteComment(t *testing.T) {
	...
	t.Run("댓글 삭제 성공 (soft delete)", func(t *testing.T) {
		...
		// 삭제된 댓글 조회하여 검증
		deleted, err := GetCommentByID(db, comment.ID)
		if err != nil {
	...
	t.Run("존재하지 않는 댓글 삭제 시 에러 반환", func(t *testing.T) {
		...
		// When: 삭제 시도
		err := DeleteComment(db, nonExistentID)
	...
	t.Run("이미 삭제된 댓글 삭제 시 에러 반환", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-already-deleted', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
...
func TestListComments(t *testing.T) {
	...
	t.Run("댓글 목록 조회 성공 (2-level 계층 구조)", func(t *testing.T) {
		...
		// 최상위 댓글 1 생성
		comment1, err := CreateComment(db, postID, nil, "Author1", "pass1", "Comment 1", "192.168.1.1", "Agent1")
		if err != nil {
			t.Fatalf("failed to create comment1: %v", err)
		}

		// 최상위 댓글 2 생성
		comment2, err := CreateComment(db, postID, nil, "Author2", "pass2", "Comment 2", "192.168.1.2", "Agent2")
		if err != nil {
			t.Fatalf("failed to create comment2: %v", err)
		}

		// 댓글 1의 대댓글 생성
		_, err = CreateComment(db, postID, &comment1.ID, "Reply1", "pass3", "Reply to 1", "10.0.0.1", "Agent3")
		if err != nil {
			t.Fatalf("failed to create reply1: %v", err)
		}

		// 댓글 2의 대댓글 생성
		_, err = CreateComment(db, postID, &comment2.ID, "Reply2", "pass4", "Reply to 2", "10.0.0.2", "Agent4")
		if err != nil {
			t.Fatalf("failed to create reply2: %v", err)
		}

		// When: 댓글 목록 조회 (limit=10, offset=0)
		comments, total, err := ListComments(db, postID, 10, 0)
	...
	t.Run("페이지네이션 동작 확인", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-pagination', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
		...
		// 5개의 최상위 댓글 생성
		for i := 1; i <= 5; i++ {
			_, err := CreateComment(db, postID, nil, fmt.Sprintf("Author%d", i), "pass", fmt.Sprintf("Comment %d", i), "192.168.1.1", "Agent")
			if err != nil {
		...
		page1, total1, err := ListComments(db, postID, 2, 0)
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if len(page1) != 2 {
		...
		page2, total2, err := ListComments(db, postID, 2, 2)
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if len(page2) != 2 {
		...
		page3, total3, err := ListComments(db, postID, 2, 4)
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if len(page3) != 1 {
	...
	t.Run("댓글이 없는 포스트 조회", func(t *testing.T) {
		...
		err := db.QueryRow(`
			INSERT INTO posts (site_id, slug, title, comment_count)
			VALUES ($1, 'test-post-empty', 'Test Post', 0)
			RETURNING id
		`, siteID).Scan(&postID)
		if err != nil {
			t.Fatalf("failed to insert test post: %v", err)
