...
// maskIPAndSetReplies는 댓글 배열의 IP 주소를 마스킹하고 대댓글도 처리합니다
// 응답 전에 호출하여 개인정보를 보호합니다
func maskIPAndSetReplies(comments []models.Comment) {
	for i := range comments {
		// 최상위 댓글의 IP 마스킹
		comments[i].IPAddressMasked = models.MaskIPAddress(comments[i].IPAddress)

		// 대댓글의 IP 마스킹
		for j := range comments[i].Replies {
			comments[i].Replies[j].IPAddressMasked = models.MaskIPAddress(comments[i].Replies[j].IPAddress)
		}
	}
}
...
func (h *CommentHandler) ListComments(w http.ResponseWriter, r *http.Request) {
	// TODO: 구현 예정
	respondJSON(w, http.StatusOK, map[string]string{"message": "ListComments - TODO"})
}
